...
# 1. 전국 광역 및 기초 지자체 데이터 (전체 데이터 포함)
REGION_DATA = {
    ...
    "울산광역시": {"code": "26", "self": "26", "sub": ["남구", "동구", "북구", "울주군", "중구"]},
...
def get_data(api_key, age, lib_code):
    ...
    try:
        res = requests.get(url, params=res_params := params).json()
...
# 1. API 키 (중간에 메시지 안 뜨게 맨 아래나 맨 위로 배치)
api_key = st.sidebar.text_input("1. API 인증키 입력", type="password", help="도서관 정보나루에서 발급받은 키를 입력하세요.")

# 2. 지역 선택 (메시지 없이 항상 노출)
